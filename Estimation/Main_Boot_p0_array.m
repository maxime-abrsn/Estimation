...
flag_graph = 0;  % note that when graph is called correlation is saved also
flag_reg = 0;   % regression empirical correl on estimated correl
flag_save = 1;  % save gmm and bootstrap results
...
gmmopt.S= 'G';				% type of spectral density matrix: W,NW, G, H, AM, P
...
gmmopt.infoz.hess = 'sd';    % available method: dfp, bfgs, gn, marq, sd
...
if flag_bootstrap ==1;
    ...
    gmmopt.prt = 1;  % no printing option doesn't work
        % (to work: need to change setting in all files. I have only changed
...
title('average beta per bin')
    ...
    path1 = ['./Graphs/EmpCorrBeta_',num2str(portfolio),'.png'];
      ...
      savefig(path3);  % saveTightFigure doesn't seem to work for fig containing many subplot
      saveas(gcf,path1);
...
%========================================================================
% Checking the match between empirical and  model values
% Graph and regression
