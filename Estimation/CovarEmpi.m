...
%%% OPTIONS %%%%
portfolio = 0 %(0) for 48FF, (1) for 100BtM (2) for beta decile, (3) for 10BtM, (4) for top 500 cap 20beta
...
datesGraph = round(trDates./10000);

figure('Position', [100, 100, 1049, 895]);
subplot(3,1,1)
plot(datesGraph,avgCorr(:,1))
title('average correlation')
legend('all stocks')
subplot(3,1,2)
plot(datesGraph,[avgCorr(:,2) avgCorr(:,end)])
title('average correlation per bin')
    legend('Low beta','High beta')
%legend('low','mid','high')
subplot(3,1,3)
plot(datesGraph,[avgBeta(:,2) avgBeta(:,end)])
title('average beta per bin')
    legend('Low beta','High beta')
if saveGraph == 1
      path1 = ['./Graphs/EmpCorrB',num2str(portfolio),'.png'];
      path3 = ['./Graphs/EmpCorrB',num2str(portfolio),'.fig'];
      savefig(path3);
      saveas(gcf,path1);
...
figure('Position', [100, 100, 1049, 250]);
plot(datesGraph,avgCorr(:,2))
hold on
plot(datesGraph,avgCorr(:,end),'k--')
title(strcat('Dataset',{' '},num2str(tmp)))
if saveGraph == 1
      path1 = ['./Graphs/EmpCorrLH',num2str(portfolio),'.png'];
      path3 = ['./Graphs/EmpCorrLH',num2str(portfolio),'.fig'];
      savefig(path3);
      saveas(gcf,path1);
      %saveTightFigure(gcf,path1);
end
...
if(portfolio ==4)
    figure('Position', [100, 100, 1049, 220]);
    plot(datesGraph,adsRep)
    title('ADS index');
    if saveGraph == 1
          path1 = ['./Graphs/adsRep.png'];
          path3 = ['./Graphs/adsRep.fig'];
          savefig(path3);
          saveas(gcf,path1);
          %saveTightFigure(gcf,path1);
    end
...
subplot(1,2,1)
    ...
    legend('All');
subplot(1,2,2)
    plot(adsRep,avgCorr(:,2),'*')
    hold on
    plot(adsRep,avgCorr(:,end),'o')
    legend('Low beta', 'High beta')

if saveGraph == 1
      path1 = ['./Graphs/EmpCorr_p',num2str(portfolio),'.png'];
      path3 = ['./Graphs/EmpCorr_p',num2str(portfolio),'.fig'];
      savefig(path3);
      saveas(gcf,path1);
      %saveTightFigure(gcf,path1);
end
...
figure('Position', [100, 100, 1049, 895]);
subplot(3,2,1)
    plot(adsRep,avgVoltot(:,1),'o')
    legend('All');
    title('Total volatility')
subplot(3,2,2)
    plot(adsRep,avgVoltot(:,2),'*')
    hold on
    plot(adsRep,avgVoltot(:,end),'o')
    legend('Low beta', 'High beta')
subplot(3,2,3)
    plot(adsRep,avgVolsys(:,1),'o')
    legend('All');
    title('Systematic volatility')
subplot(3,2,4)
    plot(adsRep,avgVolsys(:,2),'*')
    hold on
    plot(adsRep,avgVolsys(:,end),'o')
    legend('Low beta', 'High beta')
subplot(3,2,5)
    plot(adsRep,avgVolidio(:,1),'o')
    legend('All');
    title('Idiosyncratic volatility')
subplot(3,2,6)
    plot(adsRep,avgVolidio(:,2),'*')
    hold on
    plot(adsRep,avgVolidio(:,end),'o')
    legend('Low beta', 'High beta')

if saveGraph == 1
      path1 = ['./Graphs/EmpVol_p',num2str(portfolio),'.png'];
      path3 = ['./Graphs/EmpVol_p',num2str(portfolio),'.fig'];
      savefig(path3);
      saveas(gcf,path1);
      %saveTightFigure(gcf,path1);
end
...
if origin_Pads==0    
    ...
    for i=1:size(adsRep,1)
        if adsRep(i,1) < 0
            pads(i,1) = 0.83 + adsRep(i)/((abs(min(adsRep)))/0.83);
        elseif adsRep(i,1) > 0
            pads(i,1) = 0.83 + adsRep(i)/((abs(max(adsRep)))/0.17);
        else
            pads(i,1) = 0.83;
        end
    end
    ...
    for i=1:size(adsRep,1)
        if adsRep(i,1) < 0
            pads(i,1) = 0.83 + (1+(adsRep(i)-min(adsRep)).^1.2/3)*adsRep(i)/((abs(min(adsRep)))/0.83);
        elseif tmp2(i,1) > 0
            pads(i,1) = 0.83 + (1+(max(adsRep)-adsRep(i)).^1.2/3)*adsRep(i)/((abs(max(adsRep)))/0.17);
        else
            pads(i,1) = 0.83;
        end
    end
...
rhv1 = repmat(padsIn(2:end),1,nBins+1) .*avgCorrIn(1:end-1,:);
rhv2 = repmat(padsIn(2:end).^2,1,nBins+1) .*avgCorrIn(1:end-1,:);
...
title('coeffcient b as a function of pabs in regression rho_t = c + b*rho_{t-1} + e')
legend('all stocks')
subplot(2,1,2)
