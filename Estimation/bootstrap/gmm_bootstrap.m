...
% THUY: correction for overlapping bootstrap


function [gmmout, gmmopt]=gmm_bootstrap(b,gmmopt,Y,X,Z,Win)
...
while i <= maxiter
  ...
  gmmopt.i = i;
  if gmmopt.prt ~= 0;     % insert by Thuy, so that no printing option can be used
...
%{
for Bootstrap, go straight to the case not-well-behaved hessian

%======================================================================
% IF HESSIAN MATRIX IS WELL BEHAVED

if gmmopt.prt ~=0;
    fprintf(gmmopt.prt,...
        '\n            USING INVERSE HESSIAN \n');
end
...
%======================================================================
% IF HESSIAN MATRIX IS NEAR SINGULAR, use pinv
...
% THUY: corrected for overlapping bootstrap
%{
Note: using Hall notation: (Hall ... = code ...)
C = term (1st calculation above)
S = inv(W) = S
M = term (2nd calculation above)
G = M
%}


l = gmmopt.infoz.l;  % block length
mT = gmmopt.infoz.mT;    % centered moment
thetahat = gmmopt.infoz.thetahat; % original estimation
Shat = gmmopt.infoz.Shat;   % estimated S at thetahat
What = Shat\eye(north);

momthat = fcnchk(gmmopt.infoz.momthat);
[mhat ehat] = feval(momthat,thetahat,gmmopt.infoz,stat,Y,X,Z);

gmmopt.infoz.jakehat='numz_hat';	
jakehat = fcnchk(gmmopt.infoz.jakehat);
Mhat = feval(jakehat,thetahat,gmmopt.infoz,stat,Y,X,Z);
...
redandunt?
    ...
    w(1:l-1) = (1:1:l-1)  / l;
    w(l:nobs-l+1) = 1;
    w(nobs-l+2 : nobs) = (nobs - (nobs-l+2:1:nobs) +1 ) / l;
    
    wf = repmat(w(1:nobs),1,size(ehat,2)) .* ehat(1:nobs);   
    mT = sum(wf) / (nobs-l+1);
%}

    h = ehat - repmat(mT,nobs,1);  % this doesn't have Z yet 

    sizez = size(Z,2);
    sizeh = size(h,2);
    ehatn = [];
    for j=1:sizeh;
        ehatn = [ehatn repmat(ehat(:,j),1,sizez)];
    end
    h = repmat(Z,1,sizeh) .* ehatn;  % size T x no of moments (with Z)
    B = 0;
    for p = (0:nobs-l);
        for j = 1:l;
            for q = 1:l;
    ...
    B = ceil(nobs/l)/(nobs-l+1) * B;

% the bootstrap covariance matrix
Chat = pinv(Mhat'*What*Mhat);
bcovhat = Chat*(Mhat'*What*Shat*What*Mhat)*Chat/nobs;	% Cov(b)
Mthetahat = (eye(north) - Mhat*Chat*Mhat'*What);
Ahat = Mthetahat*Shat*B*Mthetahat'/nobs;		% Cov(m) - note the correction term B
...
% J stat corrected for bootstrap
gmmout.Jbooti = nobs*m'*pinv(nobs*varm)*Ahat*m;  

% corrected bcov
D = Chat * Mhat' * What * B * What * Mhat * Chat; % ie. adjust (nobs * bcov)
correction = sqrt(diag(Chat)./ diag(D));

atstat = correction .* (b - thetahat) ./ sqrt(diag(bcov));
atstat = abs(atstat);
